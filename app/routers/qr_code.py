...
from pathlib import Path  # Necessary for file path operations
import logging
...
async def create_qr_code(request: QRCodeRequest, token: str = Depends(oauth2_scheme)):
    ...
    qr_code_full_path = Path(QR_DIRECTORY) / qr_filename  # Ensure path operations are handled with Path
...
async def list_qr_codes_endpoint(token: str = Depends(oauth2_scheme)):
    ...
    responses = [QRCodeResponse(
        ...
        qr_code_url=decode_filename_to_url(qr_file[:-4]),  # Assuming '.png' is 4 characters
